...
function bind( /* path, target, verb, options */ ) {
  ...
  // Bind a middleware function directly
    ...
    const wrappedTarget = (...args) => {
      const results = target(...args)
      if (results && results.catch) {
        results.catch((err) => sails.log.error(err))
      }
    }
    bindFunction.apply(this, [path, wrappedTarget, verb, options]);
  }
...
function bindAction(path, target, verb, options) {
  ...
  middlewareToRun = _.uniq(middlewareToRun);
  // Bind each middleware to the identity.
  _.each(middlewareToRun, function(middleware) {
  ...
  bind.apply(this,[path, function(req, res, next) {
    ...
    if (!_.isFunction(sails._actions[actionIdentity])) {
      throw new Error('The requested route (' + req.method + ' ' + req.path + ') is bound to action `' + actionIdentity + '`, but no such action has been loaded.');
    }
